...
var MysqlDriver = Base.extend({
  ...
  createDatabase: function(dbName, options, callback) {
    ...
    if (typeof(options) === 'function')
      ...
      if (options.characterSet) {
        spec += 'CHARACTER SET = ' + options.characterSet;
      }
      if (options.collate) {
        spec += ' COLLATE = ' + options.collate;
      }
  ...
  getColumns: function(table, callback) {
    return this.runSql('SHOW COLUMNS FROM ' + table, callback);
  },
...
MysqlDriver.prototype = Promise.promisifyAll(MysqlDriver.prototype);
...
module.exports = function(db) {
  var intern = {
    mod: {
      ...
      log: {
        sql: function() {
          debug('SQL-OUT:', Array.prototype.slice.call(arguments).join(' '));
        },
        verbose: function() {
        },
        warn: function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    var instance = new MysqlDriver(db);

    for (var method in instance) {
      if (method.indexOf('Async') !== -1) {
        var syncName = method.replace('Async', 'Sync');
        var methodName = method.replace('Async', '');
        instance[syncName] = instance[methodName].sync.bind(instance[methodName], instance);
      }
    }
